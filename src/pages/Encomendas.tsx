...
export const Encomendas: React.FC = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let result = products.filter(p => p.ativo && p.preço === 0 && p.estoque === 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-6 mb-8 gap-4">
        <div>
          <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-brand-blue-dark block">Fragrâncias Exclusivas</span>
          <h2 className="text-2xl font-light tracking-widest text-slate-800 uppercase mt-0.5">Sob Encomenda</h2>
          <p className="text-[11px] text-slate-400 font-mono uppercase mt-1">
            Exibindo {filteredAndSortedProducts.length} de {products.filter(p => p.ativo && p.preço === 0 && p.estoque === 0).length} perfumes para encomenda
          </p>
        </div>
